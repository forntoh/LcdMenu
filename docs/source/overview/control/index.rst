...
The back controll can be bound to a menu item, allowing you to navigate back to the previous menu screen without having to use a dedicated "Back" button.

You can perform an action on the meny by calling the ``process(cmd)`` method on the menu object, where ``cmd`` is the control command you want to execute.
